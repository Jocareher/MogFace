...
parser.add_argument(
    ...
    default=28,
    type=int,
    help="Batch size of all GPUs for training",
)
parser.add_argument(
    "--num_workers",
    "-n",
...
parser.add_argument(
    "--config",
    "-c",
    default="configs/base_setting/config.yml",
    type=str,
    help="config yml.",
)
parser.add_argument(
    "--resume_iter", "-r", default=None, type=int, help="Resume from checkpoint"
)
...
if __name__ == "__main__":
    ...
    if args.resume_iter is not None:
        model_name = os.path.join(
            snapshots_dir, "model_{}000.pth".format(args.resume_iter)
        )
    ...
    for iter_idx in range(start_iter, cfg.max_iterations + 1):
        ...
        with torch.no_grad():
            ...
            if use_ali_ams:
                bbox_labels_list = [
                    Variable(bbox_label.cuda()) for bbox_label in bbox_labels_list
                ]
        ...
        if iter_idx % cfg.log_smooth_window == 0:
            print(
                "Epoch: {}, Iter: {}, time: {:.4f}s, data aug time: {:.4f}s".format(
                    cur_epoch_num, iter_idx, t2 - t1, t3 - t1
                )
            )
            if use_hcam:
                print(
                    "Loss conf: {:.4f} Loss loc: {:.4f} Loss_hcam: {:.4f}".format(
                        cls_loss.data, loc_loss.data, cls_loss_1.data
                    )
                )
            else:
                print(
                    "Loss conf: {:.4f} Loss loc: {:.4f}".format(
                        cls_loss.data, loc_loss.data
                    )
                )
        ...
        if iter_idx % cfg.snapshot_iter == 0 and iter_idx != start_iter:
            if iter_idx == 0:
                ...
                save_model_name = os.path.join(
                    snapshots_dir, "model_{}.pth".format(iter_idx)
                )
        ...
        if iter_idx in cfg.eval_iter_list:
            ...
            os.system(
                "CUDA_VISIBLE_DEVICES={} python test_single.py -n {} -c {}".format(
