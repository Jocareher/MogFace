    ...
    def forward(self, conf, loc, bbox_targets, anchors, bbox_labels_list):
        ...
        with torch.no_grad():
            for batch_idx in range(num_batch):
                ...
                mask_ig_bbox_target = bbox_targets[batch_idx][pos_anchor_idx][
                    ig_idx
                ].squeeze(1)[:, :4]

                decoded_boxes = anchor_utils.decode(
                    mask_ig_bbox_target,
                    transformed_anchors[pos_anchor_idx][ig_idx].squeeze(1),
                )
                all_decoded_boxes = anchor_utils.decode(
                    loc[batch_idx, :], transformed_anchors
                )
                p2_decoded_boxes = anchor_utils.decode(
                    bbox_targets[batch_idx][:25600][pos_anchor_idx[:25600]].squeeze(1)[
                        :, :4
                    ],
                    transformed_anchors[:25600][pos_anchor_idx[:25600]].squeeze(1),
                )
                p3_decoded_boxes = anchor_utils.decode(
                    bbox_targets[batch_idx][25600:32000][
                        pos_anchor_idx[25600:32000]
                    ].squeeze(1)[:, :4],
                    transformed_anchors[25600:32000][
                        pos_anchor_idx[25600:32000]
                    ].squeeze(1),
                )
                p4_decoded_boxes = anchor_utils.decode(
                    bbox_targets[batch_idx][32000:33600][
                        pos_anchor_idx[32000:33600]
                    ].squeeze(1)[:, :4],
                    transformed_anchors[32000:33600][
                        pos_anchor_idx[32000:33600]
                    ].squeeze(1),
                )
                p5_decoded_boxes = anchor_utils.decode(
                    bbox_targets[batch_idx][33600:34000][
                        pos_anchor_idx[33600:34000]
                    ].squeeze(1)[:, :4],
                    transformed_anchors[33600:34000][
                        pos_anchor_idx[33600:34000]
                    ].squeeze(1),
                )
                p6_decoded_boxes = anchor_utils.decode(
                    bbox_targets[batch_idx][34000:34100][
                        pos_anchor_idx[34000:34100]
                    ].squeeze(1)[:, :4],
                    transformed_anchors[34000:34100][
                        pos_anchor_idx[34000:34100]
                    ].squeeze(1),
                )
                p7_decoded_boxes = anchor_utils.decode(
                    bbox_targets[batch_idx][34100:][pos_anchor_idx[34100:]].squeeze(1)[
                        :, :4
                    ],
                    transformed_anchors[34100:][pos_anchor_idx[34100:]].squeeze(1),
                )

                uniqe_gt_bboxes = torch.unique(decoded_boxes, dim=0)
                unique_gt_bboxes_list = []
                p2_unique_gt_bboxes = torch.unique(
                    p2_decoded_boxes, dim=0, return_counts=True
                )
                p3_unique_gt_bboxes = torch.unique(
                    p3_decoded_boxes, dim=0, return_counts=True
                )
                p4_unique_gt_bboxes = torch.unique(
                    p4_decoded_boxes, dim=0, return_counts=True
                )
                p5_unique_gt_bboxes = torch.unique(
                    p5_decoded_boxes, dim=0, return_counts=True
                )
                p6_unique_gt_bboxes = torch.unique(
                    p6_decoded_boxes, dim=0, return_counts=True
                )
                p7_unique_gt_bboxes = torch.unique(
                    p7_decoded_boxes, dim=0, return_counts=True
                )
                ...
                for layer_idx, gt_bboxes in enumerate(unique_gt_bboxes_list):
                    ...
                    for tmp_idx, gt_bbox in enumerate(pure_gt_bboxes):
                        ...
                        top_k = max_num
                        l2_distance, candidate_idx = anchor_utils.get_k_center_anchor(
                            gt_bbox, transformed_anchors[start_idx:end_idx, :], top_k
                        )
                        top_k_2 = max_num - num_matched_anchor
                        relabel_idx = candidate_idx[
                            F.sigmoid(
                                conf[batch_idx, start_idx:end_idx, :][candidate_idx]
                            ).sort(0)[1][-top_k_2:]
                        ].squeeze(1)
                        relabel_idx += start_idx
                        relabel_idx = relabel_idx[labels[batch_idx, relabel_idx] != 1]
                        encode_loc = anchor_utils.encode_opr(
                            transformed_anchors[relabel_idx], gt_bbox.unsqueeze(0)
                        )
