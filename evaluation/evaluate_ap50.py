...
def evaluation_ap50(pred, gt_path, iter, det_result_txt=None):
        ...
        for i in pbar:
            ...
            for j in range(len(img_list)):
                ...
                pred_recall, proposal_list = image_eval(
                    pred_info, gt_boxes, ignore, iou_th
                )
    ...
    fo_res.write(
        "=================Iter : {} k  Results ====================".format(iter) + "\n"
    )
    fo_res.write("Test Method: {}".format(test_method) + "\n")
    fo_res.write(
        "Iter:" + " " + str(iter) + "k" + " Easy  Val AP: {}".format(aps[0]) + "\n"
    )
    fo_res.write(
        "Iter:" + " " + str(iter) + "k" + " Medium Val AP: {}".format(aps[1]) + "\n"
    )
    fo_res.write(
        "Iter:" + " " + str(iter) + "k" + " Hard Val AP: {}".format(aps[2]) + "\n"
    )
    fo_res.write("\n")
    fo_res.close()
    return aps
