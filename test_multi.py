...
parser.add_argument(
    "--num_iter", "-n", default=140, type=int, help="number of iteration for test."
)
...
parser.add_argument(
    "--max_img_shrink", default=2.6, type=float, help="constrain the max shrink of img."
)
parser.add_argument("--vote_th", default=0.6, type=float, help="bbox vote threshold")
parser.add_argument(
    "--config", "-c", default="./config.yml", type=str, help="config yml."
)
parser.add_argument(
    "--sub_project_name", default=None, type=str, help="sub_project_name."
)
parser.add_argument(
    "--test_min_scale", default=0, type=int, help="the min scale of det bbox"
)
...
def detect_face(image, shrink):
    ...
    anchors = anchor_utils.transform_anchor(
        (val_set.generate_anchors_fn(height, width))
    )
...
if __name__ == "__main__":
    ...
    for img, img_name, img_dir_name in tqdm(val_set_iter):
        ...
        with torch.no_grad():
            max_im_shrink = (
                0x7FFFFFFF / 200.0 / (img.shape[0] * img.shape[1])
            ) ** 0.5  # the max size of input image for caffe
            max_im_shrink = (
                args.max_img_shrink if max_im_shrink > 2.2 else max_im_shrink
            )
    ...
    easy_ap, medium_ap, hard_ap = evaluation_ap50(
        pred, gt_path, test_iter, det_result_txt
    )
    print(
        "Test Iter: {}, Easy: {}, Medium: {}, Hard: {}".format(
            test_iter, easy_ap, medium_ap, hard_ap
        )
    )
