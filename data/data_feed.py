...
class TrainFeed(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.add_dsfd_label:
            ...
            return (
                torch.from_numpy(img).permute(2, 0, 1),
                torch.from_numpy(encode_bbox_labels),
...
class BBoxAnchorTrainCollate(object):
    ...
    def __call__(self, batch):
        ...
        return (
            torch.stack(img_list, 0),
            torch.stack(encode_bbox_labels_list, 0),
            sample[2],
            bbox_labels_list,
        )
