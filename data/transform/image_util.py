...
def mog_sse(settings, bbox_labels, sample_bbox_scale):
    if hasattr(settings, "{}_scale_range".format(settings.main_pyramid_layer)):
        ...
        main_scale_range = getattr(
            settings, "{}_scale_range".format(settings.main_pyramid_layer)
        )
        main_start_scale = main_scale_range[0]
        main_end_scale = main_scale_range[1]

        aux_scale_range = getattr(
            settings, "{}_scale_range".format(settings.auxiliary_pyramid_layer)
        )
...
def random_brightness(img, settings):
    ...
    if prob < settings.brightness_prob:
        delta = np.random.uniform(-settings.brightness_delta, settings.brightness_delta)
        img += delta
    return img
...
def random_contrast(img, settings):
    ...
    if prob < settings.contrast_prob:
        delta = np.random.uniform(
            settings.contrast_lower_delta, settings.contrast_upper_delta
        )
        img *= delta
    return img
...
def random_saturation(img, settings):
    ...
    if prob < settings.saturation_prob:
        ...
        img[:, :, 1] *= np.random.uniform(
            settings.saturation_lower_delta, settings.saturation_upper_delta
        )
...
def das_sample_bbox(img, bbox_labels, settings):
    ...
    if num_gt == 0:
        ...
        if sample_bbox_scale == 0:
            ...
            ratio = (
                float(target_anchor) / sample_bbox_scale * (2 ** random.uniform(-1, 1))
            )
        ...
        img = cv2.resize(
            img, None, None, fx=ratio, fy=ratio, interpolation=interp_method
        )
...
def das_crop_img(img, bbox_labels, sampled_bboxes, settings):
    ...
    if sampled_bbox[0] < 0 or sampled_bbox[1] < 0:
        new_img_width = (
            img_width if sampled_bbox[0] >= 0 else img_width - sampled_bbox[0]
        )
        new_img_height = (
            img_height if sampled_bbox[1] >= 0 else img_height - sampled_bbox[1]
        )
        ...
        if (
            current_img_h < settings.sample_bbox_height
            or current_img_w < settings.sample_bbox_width
        ):
            img_pad_temp = np.zeros(
                (settings.sample_bbox_height, settings.sample_bbox_width, 3)
            )
            img_pad_temp[:, :, :] = settings.img_mean
            img_pad_temp[0:current_img_h, 0:current_img_w, :] = img_pad[
                start_top:end_bottom, start_left:end_right, :
            ].copy()
            current_img = img_pad_temp.copy()

        bbox_labels = np.concatenate(
            [current_bboxes, current_labels[:, np.newaxis]], axis=1
        )

        return current_img, bbox_labels

    current_img = img[
        sampled_bbox[1] : sampled_bbox[3], sampled_bbox[0] : sampled_bbox[2], :
    ].copy()
    ...
    if (
        current_img_h < settings.sample_bbox_height
        or current_img_w < settings.sample_bbox_height
    ):
        img_pad_temp = np.zeros(
            (settings.sample_bbox_height, settings.sample_bbox_width, 3)
        )
        img_pad_temp[:, :, :] = settings.img_mean
        img_pad_temp[0:current_img_h, 0:current_img_w, :] = img[
            sampled_bbox[1] : sampled_bbox[3], sampled_bbox[0] : sampled_bbox[2], :
        ].copy()
        current_img = img_pad_temp.copy()
...
def resize_img(
    img, bbox_labels, settings, resize_height=None, resize_width=None, resize_ratio=None
):
    ...
    if resize_height is None:
        ...
        img = cv2.resize(img, None, fx=ratio, fy=ratio, interpolation=interp_method)
    if bbox_labels.shape[0] == 0:
        return img, bbox_labels
    else:
        if resize_height is None:
