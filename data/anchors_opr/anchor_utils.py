...
def transform_anchor_opr(anchors):
    ...
    """
    return torch.cat(
        ((anchors[:, :2] + anchors[:, 2:]) / 2, anchors[:, 2:] - anchors[:, :2] + 1), 1
    )
...
def transform_anchor(anchors):
    ...
    """
    return np.concatenate(
        ((anchors[:, :2] + anchors[:, 2:]) / 2, anchors[:, 2:] - anchors[:, :2] + 1),
        axis=1,
    )
...
def ig_outlier_negative_anchor(gt_bbox, transformed_anchors, ratio=1):
    ...
    scale = (
        torch.sqrt(
            torch.abs((gt_bbox[2] - gt_bbox[0] + 1) * (gt_bbox[3] - gt_bbox[1] + 1))
        )
        * ratio
    )
    center_gt_bbox = transform_anchor_opr(gt_bbox.unsqueeze(0))
    l2_distance = torch.sqrt(
        ((transformed_anchors[:, :2] - center_gt_bbox[:, :2]) ** 2).sum(1)
    )
...
def compute_center_distance(gt_bboxes, transformed_anchors):
    ...
    l2_distance = torch.sqrt(
        (
            (
                transformed_anchors[:, :2].unsqueeze(1)
                - center_gt_bboxes[:, :2].unsqueeze(0)
            )
            ** 2
        ).sum(2)
    )
