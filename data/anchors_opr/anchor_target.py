...
class AnchorTarget(object):
    ...
    def __call__(self, anchors, bbox_labels):
        ...
        overlaps = bbox_utils.bbox_overlap(
            anchors, bbox_labels[:, :4], self.nomalized_compute_overlap
        )
        # from anchor perspective [num_anchors, 1]
        argmax_overlaps_for_anchor = overlaps.argmax(axis=1)
        max_overlaps_for_anchor = overlaps[
            [True] * overlaps.shape[0], argmax_overlaps_for_anchor
        ]
        ...
        bbox_targets = anchor_utils.encode(
            anchor_utils.transform_anchor(anchors), anchor_matched_gt
        )
