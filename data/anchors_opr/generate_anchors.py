...
class GeneartePriorBoxes(object):
    ...
    def __call__(self, img_height, img_width):
        ...
        normalized_anchor_arr = anchor_utils.normalize_anchor(final_anchor_arr).astype(
            "float32"
        )
...
if __name__ == "__main__":
    ...
    for img, _, bbox_labels in wider_face:
        ...
        np_t2 = time.time()
        print(
            "np_ssd_time gen_anchor: {} anchor_target: {} total_time: {}".format(
                np_t1 - np_t0, np_t2 - np_t1, np_t2 - np_t0
            )
        )
        np_t0 = time.time()
        faster_final_anchor_arr = generate_fpn_anchors_opr(
            settings.resize_height, settings.resize_width
        )
        np_t1 = time.time()
        np_t2 = time.time()
        print(
            "np_faster_time gen_anchor: {} anchor_target: {} total_time: {}".format(
                np_t1 - np_t0, np_t2 - np_t1, np_t2 - np_t0
            )
        )
