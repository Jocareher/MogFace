...
class WiderFaceTrainSet(object):
    ...
    def __init__(
        ...
        self.anchors = self.generate_anchors_fn(
            self.data_aug_settings.resize_height, self.data_aug_settings.resize_width
        )
        if self.out_dsfd_label:
            ...
            self.dsfd_anchors = generate_dsfd_anchors_fn(
                self.data_aug_settings.resize_height,
    ...
    def __getitem__(self, gt_idx):
        ...
        if self.data_aug_settings.use_mst:
            img, bbox_labels = self.preprocess_fn(
                img, bbox_labels, self.phase, self.data_aug_settings
            )
        else:
        ...
        encode_bbox_labels = self.anchor_target_fn(
            self.anchors, bbox_labels.astype("float32")
        )
        # self.gt_idx += 1

        if self.out_dsfd_label:
            encode_dsfd_bbox_labels = self.anchor_target_fn(
                self.dsfd_anchors, bbox_labels.astype("float32")
            )
    ...
    def validate(self, num_validated_img=100, shuffle=True):
        ...
        print(
            "validate img, bbox_labels by randomly samlping {} img and saved in {}.".format(
                num_validated_img, self.debug_img_dir
            )
        )
        ...
        for idx, item in enumerate(gt_list):
            ...
            draw_bboxes(
                img,
                np.array(bbox_labels)[:, :4],
                label_list,
                output_dir=self.debug_img_dir,
                save_img_name="or_img_{}_{}".format(
                    np.random.uniform(0, 1), img_name.split("/")[-1]
                ),
            )
            ...
            draw_bboxes(
                ...
                label_list,
                output_dir=self.debug_img_dir,
                save_img_name="data_aug_img_{}_{}".format(
                    np.random.uniform(0, 1), img_name.split("/")[-1]
                ),
            )
            ...
            decode_bboxes = anchor_utils.decode(
                torch.Tensor(encode_bbox_labels[:, :4]),
            ...
            draw_bboxes(
                img,
                np.array(decode_bboxes_pos),
                output_dir=self.debug_img_dir,
                save_img_name="decode_img_{}_{}".format(
                    np.random.uniform(0, 1), img_name.split("/")[-1]
                ),
            )
        ...
        for i in range(len(lines_input_txt)):
            ...
            if "--" not in line_txt:
                if len(line_txt) > 6:
                    ...
                    h = float(split_str[3])
                    line_txt = (
                        str(x1_min) + " " + str(y1_min) + " " + str(w) + " " + str(h)
                    )
                    file_dict[num_class].append(line_txt)
                else:
                    file_dict[num_class].append(line_txt)
